...
export const useCardanoExplorer = () => {
  ...
  if (!context) {
    throw new Error(
      "useCardanoExplorer must be used within a CardanoExplorerProvider",
    );
  }
  return context;
};
...
export const CardanoExplorerProvider: React.FC<
  PropsWithChildren<CardanoExplorerProps>
> = ({ children }) => {
  ...
  const cexplorer: CardanoExplorer = {
    mintPolicy: (policyId: string) =>
      `https://${explorerUrl}/policy/${policyId}${mintUrl}`,
