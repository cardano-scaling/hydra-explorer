...
export const useNetworkContext = () => {
  ...
  if (!context) {
    throw new Error(
      "useNetworkContext must be used within a NetworkSettingProvider",
    );
  }
  return context;
};

const NetworkSettingProvider: React.FC<any> = ({ children }) => {
  const searchParams = useSearchParams();
  const currentNetwork =
    Number(searchParams.get("network")) || mainnetNetworkMagic;
