...
const StatTile: React.FC<StatTileProps> = ({ label, value, subInfo, isLoading, hasSubInfo }) => {
    ...
    return (
        <div className="bg-card border border-border rounded-lg py-5 px-6 flex flex-col justify-between min-h-[96px] h-full transition-colors">
            ...
            {subInfo && (
                <div className="mt-2 text-[12px] text-muted-foreground leading-normal">
                    {subInfo}
                </div>
            )}
        </div>
...
const HeadsDashboard = () => {
    ...
    const subInfoLatestBlock = useMemo(() => {
        ...
        return (
            <div className="flex flex-wrap gap-x-2 gap-y-1">
                <span>Slot: <span className="font-mono">{displaySlot}</span></span>
                {blockHash && (
    ...
    return (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 w-full mb-6">
            <StatTile
                label="Total Heads"
                value={displayTotalHeads}
                isLoading={showHeadsLoading}
            />
            <StatTile
                label="Active Heads"
                value={displayActiveHeads}
                isLoading={showHeadsLoading}
            />
            <StatTile
                label="TVL (₳)"
            ...
            <StatTile
                ...
                subInfo={subInfoLatestBlock || (showTicksLoading ? undefined : <span className="text-[12px] text-muted-foreground">—</span>)}
